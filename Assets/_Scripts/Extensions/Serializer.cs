...
public static class Serializer
{
    public static void Save<T>(T toSave, string fileName, string path = "")
    {
        ...
        FileStream file = new FileStream(Path.Combine(savePath, fileName), FileMode.Create);
    ...
    public static bool TryLoad<T>(out T toLoad, string fileName, string path = "")
    {
        ...
        if (File.Exists(Path.Combine(savePath, fileName)))
        {
            ...
            BinaryFormatter bf = new BinaryFormatter();
            FileStream stream = new FileStream(Path.Combine(savePath, fileName), FileMode.Open);
